...
export default function IssueListPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Nonconformity" title="부적합 이력" description="DB에 저장된 환경 부적합과 품질 부적합을 통합 조회합니다." />{error && <div className="notice-box">{error}</div>}<div className="tab-row"><button className={type === "all" ? "active" : ""} onClick={() => handleTab("all")}>전체</button><button className={type === "env" ? "active" : ""} onClick={() => handleTab("env")}>환경 부적합</button><button className={type === "quality" ? "active" : ""} onClick={() => handleTab("quality")}>품질 부적합</button></div><div className="panel table-panel"><table className="data-table issue-list-table"><colgroup><col className="col-date" /><col className="col-status" /><col className="col-zone" /><col className="col-batch" /><col className="col-item" /><col className="col-value" /><col className="col-standard" /><col className="col-status" /><col className="col-status" /><col className="col-small" /><col className="col-action" /></colgroup><thead><tr><th>발생일시</th><th>유형</th><th>구역</th><th>배치</th><th>항목</th><th>측정값</th><th>기준</th><th>심각도</th><th>상태</th><th>알림</th><th>관리</th></tr></thead><tbody>{rows.map((issue) => <tr key={`${issue.issueType}-${issue.issueId}`}><td>{issue.occurredAt}</td><td>{issue.issueType === "env" ? "환경" : "품질"}</td><td>{issue.zoneName}</td><td className="text-left"><button className="link-cell" onClick={() => navigate(`/issues/${issue.issueType}/${rawId(issue)}`)}><strong>{issue.batchName}</strong></button></td><td className="text-left"><strong>{issue.itemName}</strong></td><td>{issue.measuredValue}</td><td className="text-left"><span className="table-text-wrap">{issue.standardRange}</span></td><td><StatusBadge value={issue.severity} /></td><td><StatusBadge value={issue.status} /></td><td>{issue.issueType === "env" ? `${issue.unreadAlertCount || 0}/${issue.activeAlertCount || 0}` : "-"}</td><td><ActionMenu items={[{ label: "상세 보기", kind: "detail", onClick: () => navigate(`/issues/${issue.issueType}/${rawId(issue)}`) }, isAdmin && { label: "삭제", kind: "delete", danger: true, onClick: () => deleteIssue(issue) }]} /></td></tr>)}</tbody></table></div></div>;
}
