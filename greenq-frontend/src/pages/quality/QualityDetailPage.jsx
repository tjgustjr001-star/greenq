...
function fmtQualityValue(item, value) {
  if (item?.itemCode === "GROWTH_STAGE") return growthStageLabelOf(value);
  return fmtValue(value, item?.unit);
}

function fmtExpectedValue(item) {
  if (!item?.expectedTextValue) return item?.standard || "-";
  const value = item.itemCode === "GROWTH_STAGE" ? growthStageLabelOf(item.expectedTextValue) : item.expectedTextValue;
  return `기대값: ${value}`;
}
...
export default function QualityDetailPage() {
  ...
  const skippedItemCount = items.filter(
    (item) => String(item.status || item.evalStatus || "").toUpperCase() === "SKIPPED"
  ).length;
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        actions={<><button className="secondary-button" onClick={() => navigate("/quality")}>목록으로</button><button className="secondary-button" onClick={() => navigate(`/quality/new?batchId=${measurement.batchId}`)}>같은 배치 새 입력</button>{isAdmin && <button className="danger-button" onClick={deleteMeasurement}>삭제</button>}</>}
      />
      ...
      <section className="stat-grid six compact quality-stat-grid">
        ...
        <div className="stat-card"><p>미입력</p><strong>{measurement.missingItemCount ?? 0}</strong><span>확인 필요</span></div>
        <div className="stat-card"><p>판정 제외</p><strong>{skippedItemCount}</strong><span>대상 아님</span></div>
