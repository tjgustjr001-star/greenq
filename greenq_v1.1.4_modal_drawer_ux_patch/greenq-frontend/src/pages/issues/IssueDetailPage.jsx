  ...
  return (
    <div className="page">
      ...
      <div className="panel detail-hero">
        ...
        <div className="inline-actions"><StatusBadge value={issue.severity} /><span className={`status-badge ${String(issue.status || "").toLowerCase()}`}>{issueStatusLabel(type, issue.status)}</span></div>
      </div>

      <section className="content-grid two">
        <div className="panel info-panel"><h3>부적합 정보</h3><dl><dt>측정값</dt><dd>{issue.measuredValue}</dd><dt>기준 최소</dt><dd>{issue.standardMin ?? "-"}</dd><dt>기준 최대</dt><dd>{issue.standardMax ?? "-"}</dd><dt>이탈률</dt><dd>{issue.deviationRate ?? "-"}</dd></dl></div>
        <div className="panel info-panel"><h3>권장 조치</h3><p>{issue.guide || issue.guideMessage || "-"}</p></div>
      </section>
      ...
      {type === "env" && (
        <div className="panel">
          ...
          {alerts.length === 0 ? <p className="muted-text">연결된 환경 알림이 없습니다.</p> : <table><thead><tr><th>생성일시</th><th>제목</th><th>수준</th><th>상태</th><th>내용</th></tr></thead><tbody>{alerts.map((alert) => <tr key={alert.alertId}><td>{alert.createdAt}</td><td>{alert.alertTitle}</td><td><StatusBadge value={alert.alertLevel} /></td><td><span className={`status-badge ${String(alert.alertStatus || "").toLowerCase()}`}>{alertStatusLabel(alert.alertStatus)}</span></td><td>{alert.alertMessage}</td></tr>)}</tbody></table>}
        </div>
      )}
      ...
      {type === "env" && (
        <div className="panel">
          ...
          {actions.length === 0 ? <p className="muted-text">등록된 조치 이력이 없습니다.</p> : <table><thead><tr><th>일시</th><th>유형</th><th>내용</th><th>상태</th><th>비고</th></tr></thead><tbody>{actions.map((action, idx) => <tr key={idx}><td>{action.actionAt}</td><td>{action.actionType}</td><td>{action.actionContent}</td><td><span className={`status-badge ${String(action.actionStatusAfter || "").toLowerCase()}`}>{issueStatusLabel("env", action.actionStatusAfter)}</span></td><td>{action.resultNote || "-"}</td></tr>)}</tbody></table>}
        </div>
      )}
