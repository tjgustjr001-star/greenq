...
public class AuthService {
    ...
    public Map<String, Object> login(LoginRequest request) {
        ...
        List<UserAccount> users = em.createQuery("select u from UserAccount u where u.loginId = :loginId", UserAccount.class)
                .setParameter("loginId", loginId)
                .getResultList();

        if (users.isEmpty()) throw new IllegalArgumentException("아이디 또는 비밀번호가 올바르지 않습니다.");
