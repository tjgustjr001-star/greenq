...
public class GreenqQueryService {
    ...
    public List<Map<String, Object>> envIssueAlerts() {
        return envIssues().stream()
                .filter(issue -> !"RESOLVED".equalsIgnoreCase(String.valueOf(issue.get("status"))))
                .sorted((a, b) -> {
                    int severityCompare = Integer.compare(severityRank(String.valueOf(b.get("severity"))), severityRank(String.valueOf(a.get("severity"))));
                    if (severityCompare != 0) return severityCompare;
                    return String.valueOf(b.get("occurredAt")).compareTo(String.valueOf(a.get("occurredAt")));
                })
                .toList();
    }
    ...
    private List<Map<String, Object>> envIssues() {
        String sql = """
                select nc.env_nc_id, nc.occurred_at, nc.resolved_at, nc.env_nc_status, nc.severity,
                       ...
                       nc.deviation_rate, nc.guide_message, b.batch_name, z.zone_name
