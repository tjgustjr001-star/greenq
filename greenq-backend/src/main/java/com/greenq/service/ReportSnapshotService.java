...
public class ReportSnapshotService {
    ...
    public ReportSnapshot build(Map<String, Object> request) {
        ...
        String envSummary = "환경 로그 " + env.totalCount() + "건, 정상 " + env.normalCount() + "건, 주의 " + env.cautionCount() + "건, 경고 " + env.failCount()
                + "건 / 평균 온도 " + fmt(env.avgTemp()) + "℃, 습도 " + fmt(env.avgHumidity()) + "%, pH " + fmt(env.avgPh()) + ", EC " + fmt(env.avgEc());
        String qualitySummary = "실측 " + quality.totalCount() + "건, 정상 " + quality.normalCount() + "건, 주의 " + quality.cautionCount() + "건, 경고 " + quality.failCount()
                + "건 / 평균 초장 " + fmt(quality.avgPlantHeight()) + "cm, 엽폭 " + fmt(quality.avgLeafWidth()) + "cm, 엽장 " + fmt(quality.avgLeafLength()) + "cm, 생체중 " + fmt(quality.avgFreshWeight()) + "g";
    ...
    private EnvStats envStats(ScopeFilter filter, LocalDateTime startAt, LocalDateTime endExclusive) {
        ...
        return new EnvStats(intVal(row[0]), intVal(row[1]), intVal(row[2]), intVal(row[3]), bd(row[4]), bd(row[5]), bd(row[6]), bd(row[7]));
    }
    ...
    private QualityStats qualityStats(ScopeFilter filter, LocalDateTime startAt, LocalDateTime endExclusive) {
        String sql = """
                ...
                """ + scopeWhere(filter);
        Object[] row = singleRow(sql, filter, startAt, endExclusive);
        return new QualityStats(intVal(row[0]), intVal(row[1]), intVal(row[2]), intVal(row[3]), bd(row[4]), bd(row[5]), bd(row[6]), bd(row[7]));
    }
    ...
    private Object[] singleRow(String sql, ScopeFilter filter, LocalDateTime startAt, LocalDateTime endExclusive) {
        ...
        if (rows.isEmpty()) return new Object[]{0, 0, 0, 0, null, null, null, null};
    ...
    private record EnvStats(int totalCount, int normalCount, int cautionCount, int failCount, BigDecimal avgTemp, BigDecimal avgHumidity, BigDecimal avgPh, BigDecimal avgEc) {}
    private record QualityStats(int totalCount, int normalCount, int cautionCount, int failCount, BigDecimal avgPlantHeight, BigDecimal avgLeafWidth, BigDecimal avgLeafLength, BigDecimal avgFreshWeight) {}
