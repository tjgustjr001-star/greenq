...
CREATE TABLE `environment_log` (
  ...
  `data_source` VARCHAR(20) NOT NULL DEFAULT 'SIMULATOR' COMMENT '데이터 출처 / SENSOR, SIMULATOR, MANUAL. 원본값 수정 불가, 삭제 시 delete_yn=Y 처리',
...
CREATE TABLE `quality_evaluation` (
  ...
  `overall_status` VARCHAR(20) NOT NULL DEFAULT 'NORMAL' COMMENT '종합 품질 판정 / NORMAL, CAUTION, FAIL, MISSING. 화면에서는 PASS/주의/경고로 표시 가능',
