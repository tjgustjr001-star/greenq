...
export default function ZoneListPage() {
  ...
  const startEdit = (zone) => { setForm(toForm(zone)); setModalOpen(true); };
  ...
  return (
    <div className="page">
      ...
      <Modal
        ...
        description="구역 기본 정보는 목록 흐름을 유지하기 위해 모달에서 등록합니다."
        onClose={closeModal}
        footer={<><button className="secondary-button" onClick={closeModal}>취소</button><button className="primary-button" onClick={saveZone}>{isEdit ? "수정 저장" : "등록"}</button></>}
      >
        <div className="form-grid modal-form-grid">
          <label>구역명<input value={form.zoneName} onChange={(e) => updateForm("zoneName", e.target.value)} placeholder="예: A구역" /></label>
          <label>위치 설명<input value={form.locationDesc} onChange={(e) => updateForm("locationDesc", e.target.value)} placeholder="예: 1동 좌측" /></label>
          <label>면적(㎡)<input value={form.areaSize} onChange={(e) => updateForm("areaSize", e.target.value)} placeholder="예: 120" /></label>
          <label>상태<select value={form.zoneStatus} onChange={(e) => updateForm("zoneStatus", e.target.value)}><option value="ACTIVE">활성</option><option value="MAINTENANCE">점검</option><option value="INACTIVE">비활성</option></select></label>
          <label className="wide-field">설명<input value={form.description} onChange={(e) => updateForm("description", e.target.value)} /></label>
        </div>
      </Modal>
