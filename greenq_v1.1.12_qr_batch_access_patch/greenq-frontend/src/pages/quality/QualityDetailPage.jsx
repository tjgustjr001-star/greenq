  ...
  return (
    <div className="page">
      <PageHeader
        ...
        actions={<><button className="secondary-button" onClick={() => navigate("/quality")}>목록으로</button><button className="secondary-button" onClick={() => navigate(`/quality/new?batchId=${measurement.batchId}`)}>같은 배치 입력</button><button className="danger-button" onClick={deleteMeasurement}>삭제</button></>}
      />
      ...
      <section className="content-grid two">
        ...
        <div className="panel"><div className="panel-head"><h3>품질 평가 기준 연결</h3></div><dl><dt>품질 평가 ID</dt><dd>{measurement.qualityEvalId || "-"}</dd><dt>평가 상태</dt><dd><StatusBadge value={measurement.overallStatus || measurement.qualityStatus} /></dd><dt>리포트 반영</dt><dd>{reflected ? "반영 완료" : "검토 후 반영 필요"}</dd><dt>검토 이력</dt><dd>{reviews.length}건</dd></dl></div>
      </section>

      <div className="panel"><div className="panel-head"><h3>품질 판정 항목</h3></div><table><thead><tr><th>항목</th><th>측정값</th><th>기준</th><th>이탈값</th><th>이탈률</th><th>상태</th></tr></thead><tbody>{items.map((item) => <tr key={item.qualityEvalItemId || item.itemCode}><td><strong>{item.itemName}</strong><br /><small>{item.itemCode}</small></td><td>{fmtValue(item.measuredValue ?? item.measuredTextValue, item.unit)}</td><td>{item.expectedTextValue ? `기대값: ${item.expectedTextValue}` : item.standard}</td><td>{item.deviationValue ?? "-"}</td><td>{item.deviationRate === null || item.deviationRate === undefined ? "-" : `${item.deviationRate}%`}</td><td><StatusBadge value={item.status || item.evalStatus} /></td></tr>)}</tbody></table></div>

      <div className="panel"><div className="panel-head"><h3>품질 부적합</h3><button className="secondary-button" onClick={() => navigate("/issues?type=quality")}>품질 부적합 목록</button></div>{qualityIssues.length === 0 ? <p className="muted-text">등록된 품질 부적합이 없습니다.</p> : <table><thead><tr><th>항목</th><th>측정값</th><th>기준</th><th>심각도</th><th>검토 상태</th><th>리포트 반영</th><th>관리</th></tr></thead><tbody>{qualityIssues.map((issue) => <tr key={issue.qualityNcId}><td>{issue.itemName}</td><td>{issue.measuredValueDisplay ?? issue.measuredTextValue ?? issue.measuredValue ?? "-"}</td><td>{issue.standardRange}</td><td><StatusBadge value={issue.severity} /></td><td>{issueStatusLabel("quality", issue.status || issue.qualityNcStatus)}</td><td>{String(issue.reportReflectedYn).toUpperCase() === "Y" ? "반영" : "미반영"}</td><td><button className="small-button" onClick={() => navigate(`/issues/quality/${issue.rawId}`)}>상세</button></td></tr>)}</tbody></table>}</div>

      <div className="panel"><div className="panel-head"><h3>품질 검토 이력</h3></div>{reviews.length === 0 ? <p className="muted-text">등록된 품질 검토 이력이 없습니다. 품질 부적합 상세에서 검토 메모를 등록할 수 있습니다.</p> : <table><thead><tr><th>검토일시</th><th>검토자</th><th>내용</th><th>상태</th><th>리포트</th></tr></thead><tbody>{reviews.map((review) => <tr key={review.qualityReviewId}><td>{review.reviewAt}</td><td>{review.reviewedByName || review.reviewedBy || "-"}</td><td>{review.reviewContent}</td><td>{issueStatusLabel("quality", review.reviewStatusAfter)}</td><td>{String(review.reportReflectedYn).toUpperCase() === "Y" ? "반영" : "미반영"}</td></tr>)}</tbody></table>}</div>

      <div className="panel"><div className="panel-head"><h3>샘플 원본</h3></div><table><thead><tr><th>No</th><th>초장</th><th>엽폭</th><th>엽장</th><th>생체중</th><th>엽색</th><th>생육단계</th><th>메모</th></tr></thead><tbody>{samples.map((sample) => <tr key={sample.sampleNo}><td>{sample.sampleNo}</td><td>{sample.plantHeight}</td><td>{sample.leafWidth}</td><td>{sample.leafLength}</td><td>{sample.freshWeight}</td><td>{sample.leafColor}</td><td>{labelOf(sample.growthStage)}</td><td>{sample.specialNote || sample.note || "-"}</td></tr>)}</tbody></table></div>
    </div>
  );
}
