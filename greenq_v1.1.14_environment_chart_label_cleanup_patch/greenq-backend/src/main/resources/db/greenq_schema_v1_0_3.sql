...
CREATE TABLE `cultivation_batch` (
  ...
  `qr_token` VARCHAR(64) NULL COMMENT 'QR 접근 토큰 / 작업자 실측 입력 진입용 배치별 토큰',
...
CREATE UNIQUE INDEX `uk_cultivation_batch_qr_token` ON `cultivation_batch` (`qr_token`);
...
('LIGHT_WAVELENGTH', '광질/파장', 'LIGHT', 'ENV', 'nm', 'TEXT', 'lightWavelength', 7),
